...
class BlockExtractorGenerator:
    ...
    def execute(self, model_, outDir_):
        ...
        for variant_i in model_.getAllVariants():
            ...
            branchInstrs = []
            for instr in variant_i.getAllInstructions():
                match instr.name:
                    case "beq"   | "bne"    | "blt" | "bge" | "bltu" | "bgeu" | \
                         "jal"   | "jalr"   | \
                         "mret"  | "sret"   | "uret" | \
                         "ecall" | "ebreak":
                        branchInstrs.append(instr.identifier)
            self.builder.setBranchInstrList(branchInstrs)

            self.__generateBlockExtractor(outDir)
            self.__generateBlockInstructionGenerator(variant_i.getAllInstructions(), outDir)
